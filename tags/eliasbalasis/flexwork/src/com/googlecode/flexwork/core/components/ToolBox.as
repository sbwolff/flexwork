	...
	public class ToolBox extends VBox
	{
		...
		private function onMouseUp(event:MouseEvent):void
		{
			if(event.target is DockableToolBarDragStrip) {
				...
				this.removeEventListener(DragEvent.DRAG_OVER, onDragOver);
//				this.removeEventListener(DragEvent.DRAG_DROP, onDragDrop);
 ...
 private function dispatchToolBarRowChangedEvent():void
		{
var toolBarRowChangedEvent:Event = new Event("toolBarRowChanged", true);
				 dispatchEvent(toolBarRowChangedEvent);
		}
		override public function addChildAt(child:DisplayObject, index:int):DisplayObject
		{
			if (child is DockableToolBar)
			{
				return addDockToolBar(DockableToolBar(child));
			} else {
				var displayObject: DisplayObject = super.addChildAt(child, index);
				dispatchToolBarRowChangedEvent();
				return displayObject;
			}
			return super.addChildAt(child, index);
		}

		private function addDockToolBar(child:DockableToolBar):DisplayObject
		{
			var row:HBox=null;
			if (!this.numChildren)
			{
		...
		private function onDragOver(event:DragEvent):void
		{
			...
			var dockToolBar:DockableToolBar=UIComponent(event.dragSource.dataForFormat("dragStrip")).parent as DockableToolBar;
