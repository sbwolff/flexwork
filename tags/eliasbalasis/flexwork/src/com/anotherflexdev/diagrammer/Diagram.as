...
            pt = event.target.localToContent(pt);
			...
			node.x = pt.x - Number(event.dragSource.dataForFormat("mouseX"));//modified by nathanleewei
			node.y = pt.y - Number(event.dragSource.dataForFormat("mouseY"));//modified by nathanleewei	
		}
