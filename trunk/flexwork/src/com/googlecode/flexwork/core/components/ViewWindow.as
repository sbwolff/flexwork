package com.googlecode.flexwork.core.components
{
	...
	import com.googlecode.flexwork.core.components.DockingWindow;
	...
	public class ViewWindow extends TabNavigator implements DockingWindow
	{
		...
		public var originalWidth:Number = -1;
		
		public var originalHeight:Number = -1;
		...
		public function ViewWindow()
		{
			...
			addEventListener(DockingEvent.REMOVING, onViewRemoving);
...
//			setStyle("borderColor", 0xACA899);			
		...
		public function saveOriginalSize():void
		{
...
//			titleBar.addEventListener(MouseEvent.DOUBLE_CLICK, onRestore);
			...
			var dockingEvent:DockingEvent=new DockingEvent(DockingEvent.MINIMIZE);
			dispatchEvent(dockingEvent);			
		}
		...
		public function fireRestore():void
		{
			...
			maximizeButton.removeEventListener(MouseEvent.CLICK, onRestore);
		...
		private function onMaximize(event:MouseEvent):void
		{
			...
			maximizeButton.removeEventListener(MouseEvent.CLICK, onMaximize);
			titleBar.removeEventListener(MouseEvent.DOUBLE_CLICK, onMaximize);
			
			maximizeButton.addEventListener(MouseEvent.CLICK, onRestore);
			titleBar.addEventListener(MouseEvent.DOUBLE_CLICK, onRestore);
...
//		private var boundsBeforeMaximize:Rectangle=new Rectangle();
		...
		public function onViewRemoving(event:DockingEvent):void
		{
			if (this.numChildren == 0)
			{
				this.parent.removeChild(this);
		...
		override public function removeChildAt(index:int):DisplayObject
		{
			...
			dispatchEvent(new DockingEvent(DockingEvent.REMOVING, true, true));
