package com.googlecode.flexwork.work.components
{
	...
	public class HierarchyView extends DockingView
	{
		...
		private function onTreeItemDoubleClick(event:ListEvent):void 
		{
			...
			if(!tree.dataDescriptor.isBranch(node)) {
				var messageEvent:MessageEvent = new MessageEvent(MessageEvent.SAMPLE_CITY_DOUBLE_CLICK);
				messageEvent.value = node;
				this.logDebug("node info:" + node);
